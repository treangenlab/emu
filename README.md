...
|--db| $EMU_DATABASE_DIR| path to emu database; directory must include the following files: species_taxid.fasta, taxonomy.tsv|
...
|--min-pid| 95| minimum percent identity (PID) based on NM tag|
|--min-align-len| 600| minimun aligned query length (excluding clipped bp)|
